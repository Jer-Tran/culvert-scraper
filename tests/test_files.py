...
def test_standard():
    # Check imports work
    content = get_files('culvert-scraper/tests/sample1')
    assert content != None
    ...
    for count, fname in enumerate(fileList):
        ...
        pname = 'culvert-scraper/tests/sample1/' + fname
...
def test_individual_file():
    fname = 'culvert-scraper/tests/sample2/fileD.jpeg'
...
def test_single_file():
    content = get_files('culvert-scraper/tests/sample2')
    assert len(content) == 1
    
    img = cv2.imread('culvert-scraper/tests/sample2/fileD.jpeg', cv2.IMREAD_GRAYSCALE)
    assert content[0].all() == img.all()

# If any of the files are not an image, then it should raise an error
def test_invalid_file_folder():
    with pytest.raises(Exception):
        get_files('culvert-scraper/tests/sample3')

def test_invalid_file():
    with pytest.raises(Exception):
        get_files('culvert-scraper/tests/test_files.py')
